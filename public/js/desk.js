...
async function loadInitialCount() {
  const pending = await fetch("/api/tickets/pending").then((resp) =>
    resp.json()
  );
...
async function finishTicket(){
    ...
    const {status, message} = await fetch(`/api/tickets/done/${workingTicket.id}`,{method:'PUT'}).then(resp=>resp.json())
    if(status==='ok'){
        workingTicket = null
